@# Included from rosidl_typesupport_gurumdds_cpp/resource/idl__dds_gurumdds__type_support.cpp.em
@{
...
header_files = [
    include_base + '/' + include_prefix + '__rosidl_typesupport_gurumdds_cpp.hpp',
    'rcutils/types/uint8_array.h',
    'rosidl_typesupport_cpp/message_type_support.hpp',
    'rosidl_typesupport_gurumdds_cpp/identifier.hpp',
    'rosidl_typesupport_gurumdds_cpp/message_type_support.h',
    'rosidl_typesupport_gurumdds_cpp/message_type_support_decl.hpp',
    'rosidl_typesupport_gurumdds_cpp/wstring_conversion.hpp',
]
}@
...
bool
ROSIDL_TYPESUPPORT_GURUMDDS_CPP_PUBLIC_@(package_name)
ros_to_dds(
...
@[        elif isinstance(member.type.value_type, AbstractWString)]@
      dds_message.@(member.name)_[i] = rosidl_typesupport_gurumdds_cpp::create_wstring_from_u16string(ros_message.@(member.name)[i]);
...
@[        elif isinstance(member.type.value_type, NamespacedType)]@
      @('::'.join(member.type.value_type.namespaces))::typesupport_gurumdds_cpp::alloc(dds_message.@(member.name)_[i]);
      if (!@('::'.join(member.type.value_type.namespaces))::typesupport_gurumdds_cpp::ros_to_dds(ros_message.@(member.name)[i], *dds_message.@(member.name)_[i])) {
        return false;
      }
@[        else]@
...
@[    elif isinstance(member.type.value_type, AbstractWString)]@
    ...
    for (size_t i = 0; i < size; i++) {
      dds_Wstring temp_wstring = rosidl_typesupport_gurumdds_cpp::create_wstring_from_u16string(ros_message.@(member.name)[i]);
...
@[    elif isinstance(member.type.value_type, NamespacedType)]@
    ...
    for (size_t i = 0; i < size; i++) {
      ...
      @('::'.join(member.type.value_type.namespaces))::typesupport_gurumdds_cpp::alloc(tmp);
      if (!@('::'.join(member.type.value_type.namespaces))::typesupport_gurumdds_cpp::ros_to_dds(ros_message.@(member.name)[i], *tmp)) {
        return false;
      }
...
@[    elif isinstance(member.type, AbstractWString)]@
    dds_message.@(member.name)_ = rosidl_typesupport_gurumdds_cpp::create_wstring_from_u16string(ros_message.@(member.name));
    if (dds_message.@(member.name)_ == NULL) {
      fprintf(stderr, "failed to convert u16string to dds_Wstring\n");
      return false;
    }
...
@[    elif isinstance(member.type, NamespacedType)]@
    if (!@('::'.join(member.type.namespaces))::typesupport_gurumdds_cpp::ros_to_dds(ros_message.@(member.name), dds_message.@(member.name)_)) {
      return false;
    }
@[    else]@
@{      assert False, "unknown type %s" % type_}@
...
@[        elif isinstance(member.type.value_type, AbstractWString)]@
      if (!rosidl_typesupport_gurumdds_cpp::convert_wstring_to_u16string(dds_message.@(member.name)_[i], ros_message.@(member.name)[i])) {
...
@[    elif isinstance(member.type.value_type, AbstractWString)]@
    for (uint32_t i = 0; i < size; i++) {
      ...
      if (!rosidl_typesupport_gurumdds_cpp::convert_wstring_to_u16string(temp_wstring, ros_message.@(member.name)[i])) {
        fprintf(stderr, "failed to convert dds_Wstring to u16string\n");
        return false;
      }
    }
...
@[    elif isinstance(member.type.value_type, NamespacedType)]@
    for (uint32_t i = 0; i < size; i++) {
      if (!@('::'.join(member.type.value_type.namespaces))::typesupport_gurumdds_cpp::dds_to_ros(
...
@[    elif isinstance(member.type, AbstractWString)]@
    if (!rosidl_typesupport_gurumdds_cpp::convert_wstring_to_u16string(dds_message.@(member.name)_, ros_message.@(member.name))) {
      fprintf(stderr, "failed to convert dds_Wstring to u16string\n");
      return false;
    }
@[    elif isinstance(member.type, BasicType)]@
...
@[    else]@
    if (!@('::'.join(member.type.namespaces))::typesupport_gurumdds_cpp::dds_to_ros(dds_message.@(member.name)_, ros_message.@(member.name))) {
      return false;
    }
@[    end if]@
@[  end if]@
...
template<>
ROSIDL_TYPESUPPORT_GURUMDDS_CPP_EXPORT_@(package_name)
const rosidl_message_type_support_t *
get_message_type_support_handle<@(__ros_msg_type)>()
{
  return &@(__ros_msg_pkg_prefix)::typesupport_gurumdds_cpp::_@(message.structure.namespaced_type.name)__handle;
}
