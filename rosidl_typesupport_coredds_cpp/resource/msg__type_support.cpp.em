// generated from rosidl_typesupport_coredds_cpp/resource/msg__type_support.cpp.em
...
@#######################################################################
@
#include "@(spec.base_type.pkg_name)/@(subfolder)/@(get_header_filename_from_msg_name(spec.base_type.type))__rosidl_typesupport_coredds_cpp.hpp"
...
#include "rosidl_typesupport_coredds_cpp/message_type_support_decl.hpp"
...
@[    else]@
    if (!@(field.type.pkg_name)::msg::typesupport_coredds_cpp::ros_to_dds(ros_message.@(field.name), dds_message.@(field.name)_))
      return false;
@[    end if]@
@[  end if]@
  }
...
@[    else]@
    for (uint32_t i = 0; i < len; i++) {
      if (!@(field.type.pkg_name)::msg::typesupport_coredds_cpp::dds_to_ros(
...
@[    else]@
    if (!@(field.type.pkg_name)::msg::typesupport_coredds_cpp::dds_to_ros(dds_message.@(field.name)_, ros_message.@(field.name))) {
      return false;
    }
@[    end if]@
@[  end if]@
...
template<>
ROSIDL_TYPESUPPORT_COREDDS_CPP_EXPORT_@(spec.base_type.pkg_name)
const rosidl_message_type_support_t*
get_message_type_support_handle<@(spec.base_type.pkg_name)::@(subfolder)::@(spec.base_type.type)>()
{
  return &@(spec.base_type.pkg_name)::@(subfolder)::typesupport_coredds_cpp::handle;
}
